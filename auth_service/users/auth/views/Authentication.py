...
import fileconnector.file_handler as LakeFH
import jwt
import datetime
...
class Register(APIView):
    ...
    def post(self, request):
        ...
        image3 = LakeFH.fileUpload(request.data["image3"],request.data["image3"].file)

        data = dict(request.data)
        data["image1"] = str(image1)
        data["image2"] = str(image2)
        data["image3"] = str(image3)
 
        ## Fixing bug when data is passed through FormData  https://developer.mozilla.org/en-US/docs/Web/API/FormData
        for i in ["first_name", "last_name", "email", "date_of_birth", "password", "sex", "country", "city"]:
            data[i] = (data[i])[0]
        logging.debug(data)
...
class RefreshToken(APIView):
    ...
    def post(self, request):
        ...
        try:
            payload = jwt.decode(token, Config.REFRESH_SECRET, algorithms=['HS256'])
